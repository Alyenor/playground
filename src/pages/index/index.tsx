...
const IndexPage = () => {
  ...
  const onRandomize = () => {
    const randomNumber = getRandomInt(paths.length)
    const path = paths[randomNumber].url
    nav(path)
  }
  ...
  return (
    <MainLayout helmet={helmet}>
      <section className="top-section bg-light-100">
        <div className="pt-28 pb-18 w-full flex items-center justify-center ">
          <div className="box p-4 h-110 w-100 bg-light-100 flex flex-col justify-center ">
            <h2 className="text-4xl font-bold tracking-widest">PLAYGROUND</h2>
      ...
      <section className="bg-teal-50 flex flex-col justify-center items-center py-6">
        <h2 className="text-xl font-bold tracking-widest my-12">SELECTED PROJECTS</h2>
        {
          paths.map((item, index) => (
            ...
            >
              <div className='flex'>
                <div className='text-sm mr-8'>{`- 0${index + 1}`}</div>
                <div className='flex-2'>
                  <h3 className="text-lg font-heavy tracking-widest mb-4 relative project-title">{item.title}</h3>
        ...
        <div className='cursor-pointer mt-14 mb-6 px-10 py-2 border-2 border-dark-400 rounded-lg hover:bg-dark-400 hover:text-light-100'>
