...
import {BackToMenuButton, Popover} from "@/components";
...
const MineSweeperPage = () => {
  ...
  const getSiblings = (i: number, j: number) => directions.map(([dx, dy]) => {
      const x2 = i + dx
      const y2 = j + dy
      if (x2 < 0 || x2 >= level + 1 || y2 < 0 || y2 >= level + 1)
        return undefined
  ...
  return <MainLayout header helmet={helmet}>
    <div className='flex flex-col items-center px-40'>
      ...
      <h1 className='font-semibold mb-2'>Game</h1>
...
const MineSweeper = React.memo(MineSweeperPage)
